...
export default function PricingCard({ plan, isPopular, billingCycle }: Props) {
  const handleGetStarted = (plan: any) => {
    ...
    const url = `https://erp.incubytes.com/login?plan=${planData}&duration=${billingCycle}&mode=register`;
    ...
    <>
      ...
      >
        <div>
          ...
          <Box p={15}>
            <Stack>
              <Text fw={700} fz="sm" className={isPopular ? 'white' : 'black'}>
                Features:
              </Text>
              <List
                spacing={8}
              ...
              >
                ...
                {plan.modules.map((mod: any, i: number) => (
                  <List.Item key={i} className={isPopular ? 'white' : 'black'}>
                    {mod.name}
                  </List.Item>
                ))}
              </List>
